...
use crate::phy::{ChecksumCapabilities, Device, DeviceCapabilities, RxToken, TxToken};
use crate::rand::Rand;
use crate::socket::*;
...
impl InterfaceInner {
        ...
        Tx: TxToken,
    {
        match packet {
            EthernetPacket::Arp(arp_repr) => {
                let ArpRepr {
                    target_hardware_addr: dst_hardware_addr,
                    ..
                } = arp_repr;
    ...
    fn has_neighbor(&self, addr: &IpAddress) -> bool {
        match self.route(addr, self.now) {
            Some(_routed_addr) => self.neighbor_cache.lookup(&_routed_addr, self.now).found(),
            None => false,
        }
    }
        ...
        Tx: TxToken,
    {
        if self.is_broadcast(dst_addr) {
            let hardware_addr = HardwareAddress::Ethernet(EthernetAddress::BROADCAST);
    ...
    fn dispatch_ip<Tx: TxToken>(
        ...
        let (HardwareAddress::Ethernet(dst_hardware_addr), mut tx_token) =
        ...
        let mut emit_ip = |repr: &IpRepr, tx_buffer: &mut [u8]| {
